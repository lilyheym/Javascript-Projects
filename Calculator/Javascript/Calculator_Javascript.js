...
function Input_Digit(digit) {
    ...
    //this checks if Wait_Second_Operand is true and sets Display_Value to the key hta was clicked on
    if (Wait_Second_Operand === true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        //This overwrites Display_Value if current value is 0, otherwise it adds onto it
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //We are saying that is the Display_Value does not contain a decimal point we want to add a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //Checks if an operator already exists and if Wait_Second_Operand id true, then updates the operator and exits from the function]
    ...
    if (First_Operand == null) {
        ...
        //If operator exists, property lookup is performed for the operator in the Perform_Calculation object and the function that matches the 
...
keys.addEventListener('click', (event) => {
    ...
    //Endures that AC cleas all input from the calculator screen
